//! Opus codec wrapper (Phase 1C.4): 48kHz mono VoIP encoder/decoder,
//! 960-sample (20ms) frames, in-band FEC, PLC on missing packets.
...
/// Opus encoder: 48 kHz mono, VoIP application, 20 ms frames.
...
impl OpusEncoder {
    /// New encoder at `bitrate` bits/second with in-band FEC enabled.
    pub fn new(bitrate: i32) -> Result<Self> {
        let mut inner = OpusEncoderInner::new(SAMPLE_RATE, Channels::Mono, Application::Voip)?;
    ...
    /// Encode one 960-sample mono `i16` frame into a fresh packet.
    ...
    pub fn encode_into(&mut self, pcm: &[i16], out: &mut Vec<u8>) -> Result<usize> {
        if pcm.len() != FRAME_SIZE {
            return Err(AudioError::Config(format!(
                "opus encoder expects {FRAME_SIZE} samples, got {}",
                pcm.len()
            )));
        }
...
/// Opus decoder: 48 kHz mono, PLC via empty-packet decode.
...
impl OpusDecoder {
    /// New 48 kHz mono decoder.
    pub fn new() -> Result<Self> {
        Ok(Self {
            inner: OpusDecoderInner::new(SAMPLE_RATE, Channels::Mono)?,
        })
    }

    /// Decode a packet into `out` (`i16` mono, at least [`FRAME_SIZE`] long).
    /// Pass `None` to conceal one lost frame (PLC). Returns samples written.
    pub fn decode(&mut self, packet: Option<&[u8]>, out: &mut [i16]) -> Result<usize> {
        if out.len() < FRAME_SIZE {
            return Err(AudioError::Config(format!(
                "opus decoder output must hold {FRAME_SIZE} samples, got {}",
                out.len()
            )));
        }
